package dev.xevy.guido.bukkit.pgm.listeners;
...
public class PGMStatsHandler implements Module {
  ...
  public PGMStatsHandler(@NonNull GuidoBukkitRuntime runtime, @NonNull String context) {
  ...
  public void onPlayerDeath(MatchPlayerDeathEvent event) {
    ...
    this.increase(victim, "deaths");
    if (killer != null) {
      this.increase(killer, "kills");
    }
  }
  ...
  public void onCoreLeakEvent(CoreLeakEvent event) {
    for (Contribution contribution : event.getCore().getContributions()) {
      this.increase(contribution.getPlayerState().getId(), "cores");
    }
  }
  ...
  public void onWoolPlaced(PlayerWoolPlaceEvent event) {
    this.increase(event.getPlayer().getId(), "wools");
  }
  ...
  public void onMonumentBroke(DestroyableDestroyedEvent event) {
    for (DestroyableContribution contribution : event.getDestroyable().getContributions()) {
      this.increase(contribution.getPlayerState().getId(), "monuments");
    }
  }
  ...
  public void onFlagCaptureEvent(FlagCaptureEvent event) {
    this.increase(event.getCarrier().getId(), "flags");
  }
    ...
    for (MatchPlayer player : event.getMatch().getPlayers()) {
      if (!event.getMatch().getWinners().isEmpty()) {
        if (this.didWin(event, player)) {
          this.increase(player.getId(), "wins");
        } else {
          if (this.isPlaying(event, player)) {
            this.increase(player.getId(), "loses");
          }
        }
      } else {
        this.increase(player.getId(), "ties");
      }
    }
    ...
    runtime
        ...
        .ifPresent(
            (connection) -> {
              this.stats.forEach(
                  (uuid, statsMap) ->
                      Requests.MinecraftLinks.saveStats(uuid, context, statsMap)
                          .future(connection));
            });
    this.stats.clear();
  }
