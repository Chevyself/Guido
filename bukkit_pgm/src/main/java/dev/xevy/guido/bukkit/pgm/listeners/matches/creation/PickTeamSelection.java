...
public class PickTeamSelection extends AbstractTeamCreation {
  ...
  @NonNull private final Map<UUID, Set<HostedPlayer>> playersLeft = new HashMap<>();

  /** The teams that were selected in the id of the match */
  @NonNull private final Map<UUID, List<SelectingTeam>> teams = new HashMap<>();
  ...
  public String getTeamName(HostedPlayer captain, @NonNull String extra) {
    String nickname = captain.getNickname();
    if (!nickname.isEmpty()) {
   ...
   */
  @NonNull
  public Team getParty(@NonNull UUID matchId, @NonNull Match match) {
   ...
   *
   * @param linkable the info to getId the player from
   * @return the player but if not online it will be null
   */
  public Player getPlayer(@NonNull MinecraftMatchTeamMember linkable) {
    return Bukkit.getPlayer(linkable.getId());
  }
   ...
   */
  private SelectingTeam getSelecting(
      @NonNull UUID matchId, @NonNull MinecraftMatchTeamMember captain) {
  ...
  public void nextPick(@NonNull UUID matchId, @NonNull SelectingTeam team) {
    ...
    if (playersLeft.isEmpty()) {
      ...
      MinecraftMatchTeamMember leader = this.getNext(matchId, team).getLeader();
   ...
   */
  public void nextLeader(@NonNull UUID matchId, @NonNull MinecraftMatchTeamMember nextLeader) {
    this.nextLeaderTask(matchId, nextLeader);
    Player player = this.getPlayer(nextLeader);
    if (player != null) {
      player.sendMessage(
          runtime
              .getLanguageHandler()
   ...
   */
  private boolean isOccupied(@NonNull UUID matchId, @NonNull Party party) {
   ...
   */
  public boolean isPicking(@NonNull UUID matchId, @NonNull MinecraftMatchTeamMember member) {
    return member.getId().equals(this.currently.get(matchId));
  }
   ...
   */
  @NonNull
  public Set<HostedPlayer> getPlayersLeft(@NonNull UUID matchId) {
   ...
   */
  @NonNull
  public List<SelectingTeam> getTeams(@NonNull UUID matchId) {
   ...
   */
  @NonNull
  public List<String> getParticipantsNames(@NonNull UUID matchId) {
  ...
  public void nextLeaderTask(@NonNull UUID id, @NonNull MinecraftMatchTeamMember leader) {
    UUID uuid = leader.getId();
            ...
            .countdown(
                ...
                (left) -> {
                  ...
                  if (secondsLeft <= 10 || secondsLeft % 5 == 0) {
                    Player player = this.getPlayer(leader);
                    if (player != null) {
                      BukkitLocaleFile locale = runtime.getLanguageHandler().getFile(player);
                      player.sendMessage(
                          locale.get(
