 ...
 *
 * @deprecated Use a regular {@link Map} or implement {@link Informative}
 */
public interface ValuesMap {
   ...
   *
   * @param key the key to getId the value from
  ...
  /**
   * The main preferences map to getId the preferences from
   *
  ...
  default boolean isSimilar(@NonNull Map<?, ?> that) {
    if (this.getMap().isEmpty() || that.isEmpty()) {
      return false;
    } else if (this.getMap().size() != that.size()) {
      return false;
    } else {
      for (String key : this.getMap().keySet()) {
        if (!that.containsKey(key)
            || that.containsKey(key) && !that.get(key).equals(this.getMap().get(key))) {
          return false;
        }
      }
      return true;
  ...
  default boolean matches(@NonNull Map<?, ?> that) {
    if (this.getMap().isEmpty() || that.isEmpty()) return false;
    for (String key : this.getMap().keySet()) {
      if (that.containsKey(key) && (that.get(key) != null && that.get(key).equals(this.get(key)))
          || (that.get(key) instanceof String
              && ((String) that.get(key)).equalsIgnoreCase(this.get(key, String.class)))) {
        return true;
      }
    }
    return false;
