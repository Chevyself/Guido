...
import me.googas.bot.core.util.Queues;
...
public class GuidoPGMQueue extends GuidoQueue {
  ...
  public QueueResult join(@NonNull Queueable queueable) {
    ...
    if (bungee == null)
      return new QueueResult(QueueResult.ActionType.JOIN, locale.get("pgm-queue.no-bungee"));
    if (!(queueable instanceof LinkableInfo))
      return new QueueResult(QueueResult.ActionType.JOIN, "Queueable must be a link");
    Linkable data = ((LinkableInfo) queueable).getLink();
    if (data == null)
      return new QueueResult(QueueResult.ActionType.JOIN, locale.get("pgm-queue.internal"));
    Collection<Linkable> links = data.getLinks(LinkableType.MINECRAFT);
    if (links.isEmpty())
      return new QueueResult(QueueResult.ActionType.JOIN, locale.get("pgm-queue.link-first"));
    MinecraftLinkable toPlay = Queues.getToPlayMinecraft(links);
    ...
    try {
      ...
      if (bol != null) {
        if (bol) {
          ...
          return new QueueResult(QueueResult.ActionType.JOIN, locale.get("pgm-queue.join-server"));
        }
      }
      return new QueueResult(
          QueueResult.ActionType.JOIN, locale.get("pgm-queue.internal-not-check"));
    } catch (MessengerListenFailException e) {
      return new QueueResult(QueueResult.ActionType.JOIN, e.getMessage());
    }
  }
