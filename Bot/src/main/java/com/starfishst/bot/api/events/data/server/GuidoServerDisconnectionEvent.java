...
/**
 * Called when a client is disconnected and the server removes it from
 * the server this means that request will be no longer accepted
 */
public class GuidoServerDisconnectionEvent extends GuidoServerEvent {
     ...
     */
    @NotNull
    private final JsonClientThread client;
     ...
     */
    public GuidoServerDisconnectionEvent(@NotNull GuidoServer server, @NotNull JsonClientThread client) {
        super(server);
        this.client = client;
    }
