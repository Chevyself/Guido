...
public abstract class TicksTracker extends PacketAdapter implements PacketHandler {
  ...
  /**
   * Increase the ticks that were sent by the player for certain amount. This will getId the UUID of
   * the player an execute {@link #increase(UUID, int)}
   *
   * @param player the player to getId the unique id and increase the ticks to
   * @param amount the amount of ticks to increase
   */
   ...
   *
   * @param uniqueId the unique id to getId the ticks from
