...
public class Mongo {
  ...
  public static GsonBuilder builderGson() {
   ...
   *
   * @param typeOfT the type of object to getId from document
   * @param document the document to getId the object from
  ...
  public static Document getQuery(
    ...
    identification.forEach((key, value) -> query.append("identification." + key, value));
    return query;
  }
  ...
  public static Document getQuery(
    ...
    recognition.forEach(
        (key, value) ->
            query.append(
                "recognition." + key, Pattern.compile(value.toString(), Pattern.CASE_INSENSITIVE)));
    return query;
  }
