...
public class GroupManagementCommands {
    ...
    @Parent
    @Command(aliases = {"groupManager", "gm"}, description = "Manages groups", node = "user:guido.group-manager")
    public Result groupManager() {
        return new Result("Group manager is used to easily edit groups thru its files");
    }

    @Command(aliases = "save", description = "Saves the group to a file in groups/<name>.json", node = "user:guido.group-manager.save")
    public Result save(@Required(name = "Group", description = "The group to save to file") Group group) {
        FileWriter writer;
        try {
            File file = CoreFiles.getOrCreate(CoreFiles.currentDirectory() + "/groups/", group.getName() + ".json");
            writer = new FileWriter(file, false);
            Mongo.GSON.toJson(group, writer);
        } catch (IOException e) {
            e.printStackTrace();
            return new Result(ResultType.ERROR, group.getName() + ".json could not be created due to an IOException.");
        }
        boolean error = false;
        try {
            writer.close();
    ...
    @Command(aliases = "load", description = "Loads a group and overrides it if it already exists", node = "user:guido.group-manager.load")
    public Result load(@Required(name = "File", description = "The file to load the group of") String fileName) {
        File file = CoreFiles.getFile(CoreFiles.currentDirectory() + "/groups", fileName);
        if (file == null) return new Result(ResultType.ERROR, "File " + fileName + " could not be found inside the `groups` directory");
        FileReader reader;
        Group group;
        try {
