...
class MatchFlowTest {
  ...
  private JsonClient setupClient(@NonNull GuidoTestRuntime runtime) throws IOException {
    return runtime.joinWithClient(
        runtime.listen(Requests.MatchServer.CAN_HOST, (context) -> true),
        runtime.listen(Requests.MatchServer.HOST, this::processHostRequest));
  }

  @NotNull
  private String processHostRequest(JClientContext context)
      throws ExecutionException, InterruptedException {
    try {
      ImmutableMinecraftMatch match =
          context.get(Requests.MatchServer.HOST_MATCH, ImmutableMinecraftMatch.class);
      Ladder ladder =
          Requests.MinecraftMatches.getLadder(match.getLadderName())
              .future(context.getMessenger())
              .get();
      Requests.MinecraftMatches.updateStatus(match.getId(), MatchStatus.STARTING)
          .future(context.getMessenger())
          .get();
      List<ImmutableMinecraftMatchTeam> requestTeams = new ArrayList<>(ladder.teamsPerMatch());
      ImmutableCollection<ImmutableMinecraftMatchTeamMember> participants = match.getParticipants();
      for (int i = 0; i < ladder.teamsPerMatch(); i++) {
        Set<ImmutableMinecraftMatchTeamMember> members = new HashSet<>(ladder.playersPerTeam());
        for (int j = 0; j < ladder.playersPerTeam(); j++) {
          members.add(participants.get(i * ladder.playersPerTeam() + j));
        }
        ImmutableMinecraftMatchTeam team =
            new ImmutableMinecraftMatchTeam(MatchTeam.NO_TEAM, members, "Team " + i);
        requestTeams.add(team);
      }
      List<ImmutableMinecraftMatchTeam> responseTeams =
          Requests.MinecraftMatches.setTeams(match.getId(), new Requests.SetTeamsData(requestTeams))
              .future(context.getMessenger())
              .get()
              .getTeams();
      Requests.MinecraftMatches.setMap(match.getId(), "Le epic map")
          .future(context.getMessenger())
          .get();
      Requests.MinecraftMatches.updateStatus(match.getId(), MatchStatus.PLAYING)
          .future(context.getMessenger())
          .get();
      Requests.MinecraftMatches.onFinish(
              match.getId(), RandomUtils.getRandom(responseTeams).getId())
          .future(context.getMessenger())
          .get();
  ...
  private Ladder setupLadder(int playersPerTeam, int teamsPerMatch) {
    ...
    return runtime
        ...
        .getGuidoGuild()
        .addLadder(
            name, playersPerTeam, 500, teamsPerMatch, 1f, 1f, MinecraftTeamSelectionType.RANDOM)
        .orElseThrow();
  }
  ...
  void matchReachesFinishedStatus() throws InterruptedException {
    ...
    Optional<? extends MinecraftMatch> match =
        fillQueue(queue, ladder.playersPerTeam() * ladder.teamsPerMatch());

    assertTrue(match.isPresent(), "A match should have been created");
    awaitClient();

    MinecraftMatch reload =
        runtime.getLoader().getMinecraftMatches().getById(match.get().getId()).orElseThrow();
    assertEquals(
        MatchStatus.FINISHED, reload.getStatus(), "Match should be FINISHED after the full flow");
  }
  ...
  void matchMapIsSet() throws InterruptedException {
    ...
    MinecraftMatch reload =
        runtime.getLoader().getMinecraftMatches().getById(match.get().getId()).orElseThrow();
    assertEquals(
        "Le epic map", reload.getMap(), "Match map should match what the server client set");
  }
  ...
  void teamsAndMembersAreCorrect() throws InterruptedException {
    ...
    assertEquals(teamsPerMatch, teams.size(), "Should have exactly " + teamsPerMatch + " teams");
  ...
  void threeVsThreeTeamSizes() throws InterruptedException {
    ...
    reload.getTeams().forEach(team -> assertEquals(playersPerTeam, team.getMembers().size()));
  }
}
